...
# The CUDA extension lives under apps/cuda_ext/ as a top-level package named
# render_cuda_ext.  Add apps/cuda_ext/ to sys.path so that imports inside
# rgbd_render resolve without hacks.
sys.path.insert(0, os.path.join(_APPS_DIR, 'cuda_ext'))
...
def _run_inference_mode(args, parser, scenes, video_images=None):
    ...
    model = _load_model_lib(
        ...
        camera_num_iterations=getattr(args, 'camera_num_iterations', 4),
    )
